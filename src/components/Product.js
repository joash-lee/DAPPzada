...
const Product = ({ item, provider, account, dappzada, togglePop }) => {
  ...
  const fetchDetails = async () => {
    const events = await dappzada.queryFilter("Buy");
    ...
    const order = await dappzada.orders(account, orders[0].args.orderId);
    setOrder(order);
  };
    ...
    <div className="product">
      <div className="product__details">
        ...
        <div className="product__order">
          ...
          <p>
            <small>Ships from </small>Dappzada
          </p>
          <p>
            <small>Sold by </small>Dappzada
          </p>
          {order && (
